 ...
 * ---------------------------------------------------------------------------
 * Codex runs this as a PreToolUse hook before every Bash command (wired up in
 * .codex/hooks.json). It receives a JSON description of the command on
 * stdin. If the command matches one of the dangerous patterns below, we tell
 * Codex to DENY it — no matter what any prompt, web page, or file tried to
 * convince it to do. That's the whole point of a hook: it's a rule the model
 ...
 *
 * This duplicates .claude/hooks/block-dangerous-commands.cjs on purpose:
 * Claude Code and Codex don't share a hook runtime, so there's nothing to
 * import from. Keep the two DANGER lists in sync when either changes.
 *
    ...
    const decision = {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          ...
          "disable this hook via .codex/hooks.json.",
      },
    };
...
// No match → print nothing, exit 0 → Codex proceeds normally.
process.exit(0);
