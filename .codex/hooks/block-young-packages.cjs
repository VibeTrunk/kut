 ...
 * ---------------------------------------------------------------------------
 * Codex runs this as a PreToolUse hook before every Bash command (wired up
 * in .codex/hooks.json), alongside block-dangerous-commands.cjs. It receives
 * a JSON description of the command on stdin.
 *
 ...
 *
 * This duplicates .claude/hooks/block-young-packages.cjs on purpose: Claude
 * Code and Codex don't share a hook runtime, so there's nothing to import
 * from. Keep the two in sync when either changes.
 *
 ...
 * To adjust it, edit THRESHOLD_DAYS below. To turn it off temporarily, edit
 * .codex/hooks.json.
 */
  ...
  if (young.length > 0) {
    return deny(
      "Blocked by your local safety hook: package(s) published less than " +
        ...
        "overriding via .codex/hooks.json."
    );
  }
