...
+3.5 combined per-session cap. Machine-readable results:
[`design/features/rating-balance.json`](design/features/rating-balance.json).

**This is a point-in-time record, no longer reproducible as written.** It was
generated by `design/features/check-rating-balance.mjs`, which loaded the
TypeScript rating engine and checked the formulas against it. Those TypeScript
formulas were deleted in ADR-064 because nothing in the app called them and
they could drift from the SQL that actually runs, so the harness was retired
with them. The numbers below stand as the analysis that informed ADR-063. If
rating balance is revisited, rebuild the harness against
