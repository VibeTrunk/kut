...
The five-feature build session those `*_NEXT_FEATURES.md` documents planned
shipped on 2026-09-06 and its rules are now in `BUILD_SPEC.md` and the ADR log,
so the plan, spec and starting prompt moved to `archive/`. Read them only for
history — where they disagree with `BUILD_SPEC.md`, the spec wins.
...
| `design/` | Rendered mockups (PNG, desktop + mobile) for the album, Chronicle and rating history, with an index mapping each to its route and spec section. Kept as the record of what was designed; all three shipped 2026-09-02, so the built app and `BUILD_SPEC.md` now win over the mockups. `design/midweek/` holds an invented Midweek Madness sample tournament with rendered reports (`node scripts/midweek/sample.mjs`), the input for that feature's design pass. | The designs change. |
...
| `DEPLOYMENTS.md` | Hosted deployment log, newest first: every migration applied to the hosted `kut` schema since 2026-08-30, with its tier, backup, `migration list` counts, smoke test and rollback. Moved out of `CLAUDE.md`, which keeps only the latest migration and the lessons that apply to every deploy. | A migration is applied to hosted (new entry at the top, and bump `CLAUDE.md`'s "Latest hosted migration" line). |
