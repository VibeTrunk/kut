...
| Plaster cast on every card | partial | **PR A done** (ADR-085, PR #107, UI only): one helper, `toLiveCardPlayer`, applies the rule on every screen that has `player_id` and `is_live`: players list and detail, Home risers, collection list, card detail, album, and the starter reveal on `/welcome` (a screen the plan below missed). `LiveCardPlayer.injured` is required and `id` is the Player id; the market and pack results pass `{ id: null, injured: false }` until PR B. KB-022 and KB-023 fixed. **Next, PR B:** steps 1 and 4 below (the migration, and the market and pack pages through the helper).<br>**Owner decisions (2026-09-23), settled in ADR-085:** (a) the market shows the cast; (b) Live cards only.<br>**Goal:** one rule for every card screen, in one place: *the cast shows on a Live card of a Player who is injured right now* (ADR-082's "Live cards", ADR-084's cast). Today each screen applies its own version, and they have drifted apart (KB-022, KB-023). Screens with a `LiveCard`: players list and detail, Home risers, collection list, card detail, album, market list and detail, pack results and reveal.<br>**Gaps today:** the market (`kut.active_market_listings`) and pack openings (`kut.my_pack_opening_results`, which feeds the reveal too) can't show the cast: neither view has `player_id` or `is_live`, and the pages pass a listing or card id as the card's id. The collection list and album ignore `is_live` (KB-022). Card detail and album hash the card id, not the Player id, so their signatures differ (KB-023).<br>**Plan (one PR, one migration):**<br>1. One additive migration appends `player_id` and `is_live` to both views. Both are ADR-079 gated, so rebase each re-emitted body on its latest version (the KB-017 / ADR-073 lesson), keep the `is_active_member()` wrapper, and append columns last. Add a database test.<br>2. One helper builds `LiveCardPlayer` from a view row and decides `injured = is_live && injuredPlayerIds.has(player_id)`; every screen uses it.<br>3. `LiveCardPlayer.injured` becomes required and `id` always means the Player id (give the shirt arc its own key), so a screen that skips the helper fails typecheck.<br>4. The market and pack pages read those views with `select("*")`: Vercel deploys before the hosted push, and naming a missing column would empty the market (PR #86, ADR-083).<br>**Cost:** a data-free additive migration with the full hosted-push routine, and a small change at about ten call sites. KB-022 and KB-023 are UI only and can ship ahead of this if wanted. |
