# KUT handoff (archived)

> **Historical — superseded.** This was the single-session handoff written
> when KUT first went live. Current orientation is in `CLAUDE.md`; the reading
> order and doc map are in `docs/README.md`; forward-looking work (including a
> de-duplicated, status-checked version of the "Recommended next phases"
> below) is in `docs/ROADMAP.md`. Kept only for history — do not treat any
> "next step" here as current. See ADR-045.
