...
export default async function AccountsPage() {
  ...
  const [profilesResponse, usersResponse, eventsResponse] = await Promise.all([
    ...
    supabase
      ...
      .select("id, target_user_id, reset_by, reason, status, created_at, completed_at")
  ...
  const emailByUserId = new Map(
    (usersResponse.data.users ?? []).map((user) => [user.id, user.email ?? "No email"]),
  );
  const accounts = (profilesResponse.data ?? [])
    ...
    .flatMap((profile) => {
      ...
      return email
        ? [{ displayName: profile.display_name, email, id: profile.id, role: profile.role }]
        : [];
    });
  const displayNameByUserId = new Map(
    (profilesResponse.data ?? []).map((profile) => [profile.id, profile.display_name]),
  );
      ...
      <section className="mx-auto max-w-xl space-y-8">
        <header className="space-y-3">
          ...
          <p className="text-ink-dim">
            Set a temporary password for a member who cannot sign in. Every attempt is recorded;
              ...
              {(eventsResponse.data ?? []).map((event) => (
                <li className="rounded-xl bg-panel p-4 text-sm text-ink-dim" key={event.id}>
                  <p className="font-semibold text-ink">
                    {displayNameByUserId.get(event.target_user_id) ?? "Unknown member"} ·{" "}
                    {event.status}
                  </p>
