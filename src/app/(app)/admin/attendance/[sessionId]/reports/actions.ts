...
export async function correctGoals(formData:FormData){
  ...
  if(!uuid.test(sessionId)||!uuid.test(playerId)||reason.length<3||reason.length>500||(!remove&&(!Number.isInteger(goals)||goals!<0||goals!>99))) throw new Error("Valid goals and a reason are required.");
