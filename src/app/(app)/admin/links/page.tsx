...
export const metadata = { title: "Account links" };
...
export default async function AccountLinksPage() {
  ...
  if (profilesRes.error || playersRes.error) {
    ...
    <main className="min-h-screen bg-board p-6 text-ink sm:p-10">
      <section className="mx-auto max-w-2xl space-y-8">
        <header className="space-y-3">
          <h1 className="text-4xl font-black tracking-tight">Account links</h1>
          <p className="text-ink-dim">
            Connect a member&rsquo;s account to their TFH player card, disable an account, or permanently delete one.
            Members are normally linked automatically when they claim an invite.
          </p>
        </header>
        <LinksTable
          accounts={accounts}
