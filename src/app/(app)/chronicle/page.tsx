    ...
    <main className="board-ground min-h-screen p-5 text-ink sm:p-10">
      <section className="mx-auto max-w-3xl space-y-12 py-4 sm:py-8">
        <header className="border-y-4 border-brass py-5">
          ...
          <p className="mt-4 text-[0.65rem] font-extrabold uppercase tracking-[0.2em] text-ink-faint">
            One issue per football week · Terrible Football Haarlem
          </p>
        </header>
        <article className="overflow-hidden rounded-3xl border border-brass-line bg-panel/70">
          <div className="space-y-4 p-8">
            <p className="text-[0.65rem] font-extrabold uppercase tracking-[0.18em] text-brass">
              Latest issue
            </p>
            <h2 className="display text-4xl">Week of {formatChronicleDate(latest.week_start)}</h2>
            <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-ink-faint">
              {formatChronicleDate(latest.week_start)} –{" "}
              {formatChronicleDate(latest.week_end, {
                day: "numeric",
                month: "long",
                year: "numeric",
              })}
          ...
          <Link
            className="flex min-h-13 items-center justify-center bg-gradient-to-b from-[#eebd63] to-[#d29a34] px-6 font-black text-ink-on-accent"
            href={`/chronicle/${latest.week_start}`}
          >
            Read the issue →
          </Link>
        </article>
        <section>
          <h2 className="display mb-4 text-3xl">Earlier issues</h2>
          <ol>
            {earlier.map((week, index) => (
              <li className="border-b border-line/40" key={week.week_start}>
                <Link
                  className="grid grid-cols-[2.5rem_minmax(0,1fr)_auto] items-center gap-4 py-4 hover:text-brass"
                  href={`/chronicle/${week.week_start}`}
                >
                  <span className="display text-2xl text-ink-faint">
                    {String(weeks.length - index - 1).padStart(2, "0")}
                  </span>
                  <span>
                    <span className="block font-black">
                      Week of {formatChronicleDate(week.week_start)}
                    </span>
