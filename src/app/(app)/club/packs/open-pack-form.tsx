...
export function OpenPackForm({
  ...
  return (
    ...
    >
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -right-20 -top-36 h-[26rem] w-[26rem] rounded-full bg-[radial-gradient(closest-side,rgba(224,172,74,0.2),transparent_70%)]"
      />
      <div className="relative">
        <input name="packSlug" type="hidden" value={packSlug} />
        <input name="expectedPrice" type="hidden" value={quotedPrice} />
        <p className="text-[0.65rem] font-extrabold uppercase tracking-[0.2em] text-brass">
          Pack store
        </p>
        <h2 className="display mt-3 text-4xl">{title}</h2>
        <p className="mt-3 max-w-md text-sm leading-relaxed text-ink-dim">
          {cardsPerPack} server-selected Live Cards. Card selection is weighted by rarity, so the
          ladder still has a top. Duplicates can happen.
        </p>
        {state.error && (
          <p className="mt-4 rounded-xl border border-brick-line/40 bg-brick-bg p-3 text-sm font-bold text-brick">
            {state.error}
          </p>
        )}
        <div className="mt-7 flex flex-wrap items-center gap-4">
          ...
          <p className="text-xs font-bold text-ink-faint">
            {affordable
              ? `${cardsPerPack} cards · leaves you ${(balance - quotedPrice).toLocaleString()}`
      ...
      {confirming && (
        <div
          className="fixed inset-0 z-50 grid place-items-end bg-board-deep/80 backdrop-blur-sm sm:place-items-center sm:p-6"
          role="presentation"
        >
          <section
            aria-labelledby={`confirm-${packSlug}`}
            aria-modal="true"
            className="w-full rounded-t-3xl border border-line bg-panel p-6 shadow-2xl sm:max-w-md sm:rounded-3xl"
            role="dialog"
          >
            <p className="text-[0.65rem] font-extrabold uppercase tracking-[0.2em] text-brass">
              {state.priceChanged ? "Price updated" : "Confirm pack"}
            </p>
            <h3 className="display mt-2 text-3xl" id={`confirm-${packSlug}`}>
              {title}
            </h3>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <button
                className="min-h-12 rounded-xl border border-line bg-panel-soft px-4 font-extrabold"
              ...
              <button
                className="min-h-12 rounded-xl bg-brass px-4 font-black text-ink-on-accent disabled:opacity-60"
                disabled={isPending || balance < quotedPrice}
                onClick={() => {
                  if (state.priceChanged) idempotencyKey.current = null;
                }}
                type="submit"
              >
