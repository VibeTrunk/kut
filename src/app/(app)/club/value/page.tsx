...
export default async function ClubValuePage() {
  ...
  return <main className="board-ground min-h-screen p-5 text-ink sm:p-10"><section className="mx-auto max-w-3xl space-y-8 py-4 sm:py-8">
    ...
    <section><p className="text-[0.65rem] font-extrabold uppercase tracking-[0.16em] text-ink-faint">How copies count</p><div className="mt-4 grid grid-cols-4 gap-2">{ECONOMY.duplicateEditionWeights.map(({label,percent})=><div className="rounded-xl border border-line p-3 text-center" key={label}><p className="text-xs text-ink-dim">{label}</p><p className="mt-2 text-xl font-black text-brass">{percent}%</p></div>)}</div><p className="mt-4 text-sm text-ink-dim">Per edition. Discard payouts stay at their full value.</p></section>
