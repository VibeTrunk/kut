...
export default async function LeaderboardPage() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "rank, display_name, club_name, club_value, card_count, unique_player_count, is_current_user",
    )
    .order("rank")
    .order("display_name");
    ...
    <main className="board-ground min-h-screen p-5 text-ink sm:p-10">
      <section className="mx-auto max-w-5xl space-y-8 py-4 sm:py-8">
        ...
        {clubs.length === 0 ? (
          ...
          <div>
            <div
              className={`grid gap-x-3 border-b border-line/60 px-3 pb-2.5 text-[0.6rem] font-extrabold uppercase tracking-[0.14em] text-ink-faint sm:gap-x-4 sm:pb-3 ${COLUMNS}`}
            >
                  ...
                  <span className="min-w-0">
                    <span className="block truncate text-[0.95rem] font-extrabold leading-tight">
                      ...
                      {club.is_current_user && (
                        <span className="ml-1.5 text-[0.6rem] font-black uppercase tracking-[0.12em] text-brass">
                          You
                        </span>
                      )}
                    </span>
