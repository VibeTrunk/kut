...
export default async function MarketPage({ searchParams }: MarketPageProps) {
  ...
  return <main className="min-h-screen bg-slate-950 p-5 text-slate-50 sm:p-10"><section className="mx-auto max-w-6xl space-y-7">
    <div className="flex justify-end"><LogoutButton /></div>
    <nav className="flex justify-between text-sm font-bold"><Link className="text-amber-400" href="/club">← My Club</Link><span className="text-slate-400">{balance} KUT Coins</span></nav>
    <header><p className="text-sm font-black uppercase tracking-[0.24em] text-amber-400">Transfer market</p><h1 className="mt-2 text-4xl font-black tracking-tight sm:text-5xl">Buy Live Cards</h1><p className="mt-3 text-slate-300">Buy-now listings from other clubs. Prices are paid in KUT Coins; a 5% tax is burned.</p></header>
    <form className="grid gap-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 sm:grid-cols-5"><input className="min-h-11 rounded-xl bg-slate-950 px-3 sm:col-span-2" defaultValue={query.q} name="q" placeholder="Search player" /><select className="min-h-11 rounded-xl bg-slate-950 px-3" defaultValue={query.rarity} name="rarity"><option value="">Any tier</option>{rarities.map((rarity) => <option key={rarity} value={rarity}>{rarity}</option>)}</select><select className="min-h-11 rounded-xl bg-slate-950 px-3" defaultValue={query.sort} name="sort"><option value="newest">Newest</option><option value="price">Price</option><option value="ovr">OVR</option></select><button className="min-h-11 rounded-xl border border-amber-400 px-4 font-black text-amber-300" type="submit">Filter</button></form>
    {listings.length === 0 ? <p className="rounded-2xl border border-dashed border-slate-700 p-8 text-center text-slate-300">No active listings match these filters. List a tradeable card from My Club to start the market.</p> : <div className="grid grid-cols-[repeat(auto-fit,minmax(230px,280px))] justify-center gap-5">{listings.map((listing) => <article className="w-full rounded-[1.25rem]" key={listing.listing_id}><LiveCard player={{ id: listing.listing_id, displayName: listing.display_name, archetype: listing.archetype, liveOvr: listing.ovr, pac: listing.pac, sho: listing.sho, pas: listing.pas, dri: listing.dri, def: listing.def, phy: listing.phy, rarityTier: listing.rarity_tier }} /><p className="mt-3 text-center text-sm font-semibold text-slate-400">Sold by {listing.seller_display_name}</p><p className="mt-1 text-center text-xl font-black text-amber-300">{listing.price} KUT Coins</p><BuyListingForm canAfford={balance >= listing.price} listingId={listing.listing_id} price={listing.price} /></article>)}</div>}
  </section></main>;
}
