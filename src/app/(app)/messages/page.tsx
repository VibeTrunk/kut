...
export default async function MessagesPage() {
  ...
  return <main className="min-h-screen bg-board p-5 text-ink sm:p-10"><section className="mx-auto max-w-3xl space-y-8">
    ...
    {messages.length === 0 ? <div className="rounded-3xl border border-dashed border-line bg-panel/60 p-8 text-center"><h2 className="text-xl font-black">Your inbox is clear</h2><p className="mt-2 text-ink-dim">Market sale and purchase messages will appear here.</p></div> : <ol className="space-y-3">{messages.map((message) => <li className={`rounded-2xl border p-5 ${message.read_at ? "border-panel-2 bg-panel/50" : "border-brass/50 bg-brass/10"}`} key={message.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-brass">{message.event_type === "market_sale" ? "Market sale" : "Market purchase"}</p><h2 className="mt-1 text-xl font-black">{message.title}</h2></div><time className="text-xs font-semibold text-ink-faint" dateTime={message.created_at}>{formatDate(message.created_at)}</time></div><p className="mt-3 text-ink-dim">{message.body}</p><div className="mt-4 flex items-center justify-between gap-3">{message.read_at ? <span className="text-xs font-bold uppercase tracking-[0.12em] text-ink-faint">Read</span> : <span className="text-xs font-bold uppercase tracking-[0.12em] text-brass">New</span>}{!message.read_at && <MarkMessageReadForm notificationId={message.id} />}</div></li>)}</ol>}
  </section></main>;
}
