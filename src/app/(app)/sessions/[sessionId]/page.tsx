...
export default async function SessionRedirect({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const supabase = await createClient(); const { data, error } = await supabase.schema("kut").from("match_sessions").select("session_date").eq("id", sessionId).eq("status", "published").maybeSingle();
  if (error) throw new Error("Could not locate this session."); if (!data) notFound();
  permanentRedirect(`/chronicle/${weekStart(data.session_date)}#s-${sessionId}`);
}
