...
export default async function SessionReportPage({ params }: Props) {
  ...
  const { data: context, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw new Error("Could not load this session report.");
  if (!context) notFound();
  const report = context as Context;
  const [categoriesResponse, attendanceResponse, kudosResponse] = await Promise.all([
    supabase
      .schema("kut")
      .from("kudos_categories")
      .select("id,title,description")
      .in("id", report.category_ids),
    supabase
      .schema("kut")
      .from("attendance")
    ...
    supabase
      .schema("kut")
      .from("session_kudos")
      .select("category_id,recipient_player_id")
      .eq("session_id", sessionId)
      .eq("nominator_player_id", report.player_id),
  ]);
  if (categoriesResponse.error || attendanceResponse.error || kudosResponse.error)
    throw new Error("Could not load report choices.");
  const order = new Map(report.category_ids.map((value, index) => [value, index]));
  ...
  return (
    <main className="board-ground min-h-screen p-5 text-ink sm:p-10">
      <section className="mx-auto max-w-2xl space-y-8 py-4 sm:py-8">
        <Link className="text-sm font-black text-brass" href="/">
          ← Home
        </Link>
        <header>
          ...
          <p className="mt-3 text-sm text-ink-dim">
            ...
            }).format(new Date(report.closes_at))}
            .
          </p>
        </header>
        {closed ? (
          <section className="rounded-3xl border border-line bg-panel/60 p-8 text-center">
            <h2 className="display text-3xl">Reports closed</h2>
            <p className="mt-3 text-ink-dim">
              {report.survey_status === "finalized"
                ? "Results are ready in the Chronicle."
                : "Results are being prepared."}
            </p>
            {report.survey_status === "finalized" && (
              <Link
                className="mt-5 inline-flex min-h-12 items-center rounded-xl bg-brass px-5 font-black text-ink-on-accent"
                href={`/chronicle`}
              >
                Open Chronicle
              </Link>
