...
export function ReportForm({
  ...
  function submit(formData: FormData) {
    ...
    formData.set("idempotencyKey", requestKey.current);
  ...
  return (
    <form action={submit} className="space-y-7">
      ...
      <input name="categoryIds" type="hidden" value={categories.map((c) => c.id).join(",")} />
      {state.saved && (
        <p
      ...
      <fieldset className="space-y-4">
        ...
        <p className="text-sm text-ink-dim">
          Choose a different teammate in each category, or explicitly Skip.
        </p>
        {categories.map((category) => (
          <label className="block rounded-2xl border border-line bg-panel/60 p-4" key={category.id}>
            ...
            <select
              ...
              defaultValue={savedNominations[category.id] ?? ""}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <button
          className="min-h-13 rounded-xl bg-brass font-black text-ink-on-accent"
          disabled={pending}
          name="intent"
          value="submit"
        >
