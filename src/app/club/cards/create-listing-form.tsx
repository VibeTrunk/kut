...
export function CreateListingForm({ cardId, minimumPrice, maximumPrice }: { cardId: string; minimumPrice: number; maximumPrice: number }) {
  ...
  return <form action={action} className="rounded-2xl border border-cyan-400/35 bg-cyan-950/20 p-4">
    ...
    <button className="mt-4 min-h-11 rounded-xl bg-cyan-300 px-4 font-black text-slate-950 disabled:bg-slate-700 disabled:text-slate-400" disabled={pending} type="submit">{pending ? "Listing..." : "Create 24-hour listing"}</button>
  </form>;
}
