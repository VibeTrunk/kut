...
export function ratingDomain(values: number[]) {
  ...
  const highest = RARITY_BANDS.findIndex(
    (band) => band.tier === getRarityTier(Math.max(...values)),
  );
...
function GoalFootball({ goals, x, y }: { goals: number; x: number; y: number }) {
  return (
    <g aria-label={`${goals} goal${goals === 1 ? "" : "s"} in this week`}>
      ...
      <path
        d={`M ${x - 5.7} ${y - 1} l 3.8 1.3 M ${x + 5.7} ${y - 1} l -3.8 1.3 M ${x} ${y + 6.4} l 0 -3.2`}
        fill="none"
        stroke="#15130f"
        strokeLinecap="round"
        strokeWidth="1.2"
  ...
  if (snapshots.length === 0) {
    return (
      <p className="text-sm font-semibold text-ink-faint">
        {playerName}&rsquo;s rating history starts with the next published session.
      </p>
    );
  }
  ...
  return (
    <section className="space-y-3" aria-labelledby="rating-history-title">
      <div className="flex items-end justify-between gap-4">
        <h2
          id="rating-history-title"
          className="max-w-[15rem] text-[0.65rem] font-extrabold uppercase tracking-[0.15em] text-ink-faint"
        >
          Rating over {snapshots.length} published {snapshots.length === 1 ? "week" : "weeks"}
        </h2>
        <p className="text-4xl font-black tabular-nums leading-none text-brass">
          {latest.live_ovr}
          <span className="ml-1 text-[0.65rem] uppercase tracking-[0.16em]">now</span>
        </p>
      </div>
      <svg
        className={`block h-auto w-full ${snapshots.length === 1 ? "rounded-2xl border border-line/60 bg-panel/40 p-2" : ""}`}
        role="img"
        aria-label={`${playerName}: ${range}, latest rating ${latest.live_ovr}.`}
        ...
        {bands.map((band) => {
          ...
          return (
            <g key={band.tier}>
              ...
              />
              <text
                className="fill-ink-faint text-[10px] font-bold uppercase tracking-[0.12em]"
                x="4"
                y={bandBottom + 3}
              >
                {band.tier} {band.min}
          ...
          return (
            <g key={snapshot.week_start}>
              ...
              {goals > 0 ? (
                ...
                <circle
                  cx={x(index)}
                  cy={y(snapshot.live_ovr)}
                  fill={index === snapshots.length - 1 ? "#e0ac4a" : "#19160f"}
                  r={index === snapshots.length - 1 ? "5" : "3.5"}
                  stroke="#e0ac4a"
                  strokeWidth="2"
                />
              )}
      ...
      {snapshots.length === 1 && (
        <p className="text-sm text-ink-dim">
          One published week so far. The line begins with the next published session.
        </p>
      )}
      {snapshots.length === 1 && nextBand && (
        <p className="text-sm font-bold text-brass">
          {nextBand.min - latest.live_ovr} OVR from{" "}
          {nextBand.tier[0].toUpperCase() + nextBand.tier.slice(1)}.
        </p>
      )}
      <table className="sr-only">
        <caption>{playerName}&rsquo;s rating history</caption>
        <thead>
          <tr>
            <th>Week</th>
            <th>OVR</th>
