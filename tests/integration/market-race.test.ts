...
describe("local two-client market race", () => {
  beforeAll(async () => {
    ...
    await admin.query("insert into kut.card_editions (id, player_id, edition_type, title, is_live, snapshot_ovr, snapshot_pac, snapshot_sho, snapshot_pas, snapshot_dri, snapshot_def, snapshot_phy, special_discard_multiplier, snapshot_archetype, snapshot_rarity_tier, description, artwork_key, artwork_version, issued_at) values ($1, $2, 'other', 'Two-client race fixture', false, 50, 50, 50, 50, 50, 50, 50, 1, 'all_rounder', 'silver', 'Market race fixture.', 'tests/market-race', 1, now())", [fixture.edition, fixture.player]);
