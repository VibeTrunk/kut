...
const fx = {
  ...
  player: "30000000-0000-4000-8000-000000000011",
  season: "30000000-0000-4000-8000-000000000021",
  session: "30000000-0000-4000-8000-000000000022",
  packKey: "30000000-0000-4000-8000-000000000031",
};
...
async function cleanup() {
  ...
  await admin.query("delete from kut.session_report_requests where user_id=any($1::uuid[])", [
    [fx.user, fx.low],
  ]);
  ...
  const openings = await admin.query(
    "select id from kut.pack_openings where user_id=any($1::uuid[])",
    [[fx.user, fx.low]],
  );
  const ids = openings.rows.map((r) => r.id);
  if (ids.length) {
    const cards = await admin.query(
      "select result.card_id,card.edition_id from kut.pack_opening_cards result join kut.user_cards card on card.id=result.card_id where result.opening_id=any($1::uuid[])",
      [ids],
    );
    for (const row of cards.rows)
      await admin.query(
        "update kut.card_editions set minted_count=greatest(0,minted_count-1) where id=$1",
        [row.edition_id],
      );
    await admin.query("delete from kut.pack_opening_cards where opening_id=any($1::uuid[])", [ids]);
    await admin.query("delete from kut.user_cards where id=any($1::uuid[])", [
      cards.rows.map((r) => r.card_id),
    ]);
    await admin.query("delete from kut.pack_openings where id=any($1::uuid[])", [ids]);
  }
...
async function begin(client: Client, user: string) {
  await client.query("begin");
  await client.query("set local role authenticated");
  await client.query("select set_config('request.jwt.claim.sub',$1,true)", [user]);
}
async function run(client: Client, sql: string, args: unknown[]) {
  try {
...
beforeAll(async () => {
  ...
  await cleanup();
  await admin.query(
    "insert into auth.users(id,email,aud,role,raw_app_meta_data,raw_user_meta_data,created_at,updated_at) values($1,'race-feature@example.test','authenticated','authenticated','{}','{}',now(),now()),($2,'race-low@example.test','authenticated','authenticated','{}','{}',now(),now())",
    [fx.user, fx.low],
  );
  await admin.query(
    "insert into kut.players(id,slug,display_name,archetype) values($1,'feature-race-player','Feature Race Player','all_rounder')",
    [fx.player],
  );
  await admin.query(
...
describe("next-feature concurrency", () => {
  ...
  it("credits one report reward across a submit race", async () => {
    await admin.query(
      "insert into kut.seasons(id,name,starts_on,is_active) values($1,'Race Reports',current_date,false)",
      [fx.season],
    );
    await admin.query(
      "insert into kut.match_sessions(id,season_id,session_date,session_type,status,published_at,rating_rules_version) values($1,$2,current_date,'other','published',now(),2)",
      [fx.session, fx.season],
    );
    await admin.query("insert into kut.attendance(session_id,player_id,goals) values($1,$2,0)", [
      fx.session,
      fx.player,
    ]);
    const cats = [
    ...
    await admin.query(
      "insert into kut.session_surveys(session_id,opened_at,closes_at,category_ids,selection_seed) values($1,now(),now()+interval '24 hours',$2,$3)",
      [fx.session, cats, "30000000-0000-4000-8000-000000000040"],
    );
    await admin.query(
      "insert into kut.session_survey_eligibility(session_id,player_id,user_id) values($1,$2,$3)",
      [fx.session, fx.player, fx.user],
    );
    const nominations = Object.fromEntries(cats.map((c) => [c, null]));
    await Promise.all([begin(a, fx.user), begin(b, fx.user)]);
    const results = await Promise.all([
      run(a, "select kut.submit_session_report($1,0,$2,0,$3,'submit') result", [
        fx.session,
        nominations,
        "30000000-0000-4000-8000-000000000041",
      ]),
      run(b, "select kut.submit_session_report($1,0,$2,0,$3,'submit') result", [
        fx.session,
        nominations,
    ...
    expect(
      (
        await admin.query(
          "select count(*)::int count from kut.session_report_rewards where session_id=$1",
          [fx.session],
        )
      ).rows[0].count,
    ).toBe(1);
    expect(
      (
        await admin.query(
          "select sum(amount) total from kut.wallet_ledger where user_id=$1 and reason='session_report_reward'",
          [fx.user],
        )
      ).rows[0].total,
    ).toBe("50");
